### 아이폰 기본 계산기 어플 로직처럼 작동하도록 사칙연산을 구현.
`+ - / x` 외에는 아직 구현하지 않아 작동 안함. 혹은 이상하게 작동됨.
...
class _CalculatorNeuAppState extends State<CalculatorNeuApp> {
  ...
  String numMem = "0";
  String opMem = "";
  String calculate(String numMem, String input, String result) {
    double res = 0;
    switch (input) {
      case '+':
        res = double.parse(numMem) + double.parse(result);
        break;
      case '-':
        res = double.parse(numMem) - double.parse(result);
        break;
  ...
  void _updateResult(String? input) {
    setState(() {
      if (result == "0") {
        result = "";
      }
      switch (input) {
        case '=':
          result = calculate(numMem, opMem, result);
          opMem = "";
          break;
        case 'C':
          result = "0";
  ...
  Widget _buttonRounded(
      ...
      Color? textColor}) {
    return InkWell(
      onTap: () => _updateResult(title.toString()),
      child: Padding(
